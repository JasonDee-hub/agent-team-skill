...
本包提供 **1 个编排 Skill + 7 位专家人设**。Cursor 用 `/agent-team`，Codex 用 `$agent-team` 或自然语言触发；人设在 `experts/`，交接格式见 [handoff.md](handoff.md)。
...
- **域外知识工作（小说/论文/纪要/方案等）不扩编花名册**；低风险短任务可免追问，其余任务按 [domain-grilling.md](domain-grilling.md) 达成共识后只派 Atlas。
